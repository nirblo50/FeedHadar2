...
public class VeganFood
{
    ...
    public String drawVeganFood (int hadarPos)
    {
        // drawing the food
        batch.begin();
        ...
        // if food was eaten
        if ( ((foodPosY) <= hadar.getHeight()) && ((foodPosX + width >= hadarPos) && (foodPosX <= hadarPos + hadar.getWidth())))
        {
            // if the food is vegan or not
            if (isVegan())
            {
                resetVeganFood();
                return  "vegan food eaten";
            }

            else
            {
                resetVeganFood();
                return  "meat food eaten";
            }
        }
        return "nothing";
    }
